...
for i in range(500):
    ...
    if base == 'Z':
        ...
        if bit == 0:
            qc.s(0)  # Apply S gate (π/2 phase shift) to put the qubit in a different superposition state if bit is 0
        else:
            qc.x(0)
            qc.s(0)  # Apply S gate (π/2 phase shift) to manipulate the state
...
for i in range(500):
    ...
    if base == 'Z':
        ...
        qc.s(0)  # Apply S gate to change from Z basis to X basis
        qc.measure(0, 0)
